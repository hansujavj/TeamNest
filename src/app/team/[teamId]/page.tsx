            ...
            <ul className="space-y-3">
              {members.map((m) => (
                <li key={m.user_id} className="flex items-center gap-3">
                  <span className="w-9 h-9 rounded-full bg-[#D4C9BE] flex items-center justify-center text-[#123458] font-bold text-lg border-2 border-[#123458] shadow">{m.profiles?.name?.[0]?.toUpperCase() || "U"}</span>
                  <div>
                ...
                return (
                  <li key={task.id} className="p-5 bg-white border border-[#D4C9BE] rounded-2xl shadow-md flex flex-col gap-2 hover:shadow-lg hover:scale-[1.01] transition-all">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {task.domain?.name && (
                        <span className="ml-2 px-2 py-0.5 rounded-full border text-xs font-semibold bg-[#D4C9BE] text-[#123458] border-[#123458]">{task.domain.name}</span>
                      )}
                    </div>
